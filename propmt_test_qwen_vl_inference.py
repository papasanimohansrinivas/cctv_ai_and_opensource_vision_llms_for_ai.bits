...
server_public_ip_address =  local_ip_address

rabbit_mq_ip = sys.argv[1]

# pipeline_management_server_ip = sys.argv[2]
pipeline_management_server_ip = rabbit_mq_ip
...
def send_alert_to_gupshup(server_public_ip_address,payload):
    resp=requests.post('http://{}:60000/fire_alert_mechanism'.format(pipeline_management_server_ip), json=payload)
    if "Started Fire alert" in str(resp.content):
        alert_web_request_started = True
    else:
        #log somewhere to track for compliance puposes , for now just print 
...
co = 0
current_total =0 
for file_1 in os.listdir("./inputs"):
    a = False
    if file_1.replace(".png","") in ["947dd81b-5ee0-4cb1-90cb-c7bfe43d8618","aac24f9a-5ab3-472c-adc0-0c061e054809","9455af6e-0d46-4f4a-8e62-55145331b8b9","c9a7ba1a-c7b8-421e-8667-6c519852eb5e","1b5f83ea-1bd3-4ab4-a1e6-32b7ac7e6e01","b9868b29-f557-4191-8082-6db3f3d9bd00"]:
        a = True
...
# while (True):
    try :
        ...
        if a:
            ...
            current_name = str(file_1.replace(".png",""))
            # cv2.imwrite("./inputs/{}.png".format(current_name),input_numpy)
            # c = time.time()
            ...
            try:
                ...
                query = tokenizer.from_list_format([
                    {'image': '/root/cctv_ai_and_opensource_vision_llms_for_ai.bits/inputs/{}.png'.format(current_name)}, # Either a local path or an url
                    {'text': 'return all the burning fires in image'},
                ])
                ...
                print(response,current_name,"first turn <<<<<<<<<<<<<<<,-------------")
                analysis_array.append([cam_no,cam_time,"first stage inference",response])
                pattern = r"<box>(.*?)</box>"
                ...
                # # try:
                # print("pinging ",'http://{}:60000/fire_alert_mechanism'.format(pipeline_management_server_ip))
                # temp_thread = Thread(target = send_alert_to_gupshup,args=(server_public_ip_address,payload,))
                # temp_thread.start()
                ######### test use cases
                if image:
                    image.save("./prompt_test_multimodal_llm_pilot_outputs/"+"{}_{}_{}_fire_pred.png".format(current_name,cam_no,str(cam_time)))
                else:
                    print("no box")
                ...
                if len(boxes)==0:
                    ...
                    {'image': '/root/cctv_ai_and_opensource_vision_llms_for_ai.bits/inputs/{}.png'.format(current_name)}, 
                        {'text': 'describe what it is in detail ? <box>{}</box>'.format(boxes[0])},
                    ...
                    image = tokenizer.draw_bbox_on_latest_picture(response2, None)
                    # if image:
                        # image.save("{}_{}_{}_fire_pred.png".format(current_name,cam_no,str(cam_time)))
                    # else:
                        # print("no box")
                    ###### quesionable prompts 
                     ### very questionable
                    #  (("doll" in response2) and ( "floor" in response2)) or \
                    ...
                    (("no" in response2) and  ("fire" in response2)) or \
                     ...
                     ("fire" in response2 and "extinguisher" in response2) or \
                     ("extinguisher" in response2) or \
                     (("rabbit" in response2 ) and ("paint" in response2) and ("wall" in response2)) or \
                     (("rabbit" in response2 ) and ("wall" in response2)) or \
                     (("rabbit" in response2 ) and ("drawn" in response2) and ("wall" in response2)) or \
                     (("rabbit" in response2) and ("drawing" in response2)) or \
                     (("stuffed" in response2) and ("animal" in response2)) or \
                     (("stuffed" in response2) and ("animal" in response2) and ("on the ground" in response2)) or \
                     (("doll" in response2) and ( "floor" in response2)) or \
                     ("doll" in response2) or \
                     (("no" in response2 ) and ("it" in response2) and ("is" in response2)) or \
                     (("no" in response2 ) and ("is" in response2)) or \
                     (("no" in response2 ) and ("it" in response2)) or \
                     (("bunny" in response2 ) and ("wall" in response2)) or \
                     ("bunny" in response2) or \
                     ("cushion" in response2) or \
                     (("false" in response2 ) and ("alaram" in response2)) or \
                     (("no" in response2 ) and ("normal fire" not in response2 )) or \
                     ("boat" in response2) or \
                     (("fake" in response2) and ("fire" in response2)) or \
                     (("false" in response2) and ("fire" in response2)) or \
                     (("plastic" in response2) and ("slide" in response2) ) or \
                     (("play" in response2) and ("area" in response2)) or \
    ...
    #                           temp_thread.start()
                            ...
                            # "cam_no":cam_no
                            # }
                            print("shit fire found : ",current_name,"running total: ",current_total,"total fifres until now :", co,"ratio : ",co/current_total)
                            co+=1
                            # payload = {'server_public_ip_address':server_public_ip_address,
                            #             "media_payload":"http://{}:5000/fire/{}_{}_{}_fire_pred.png".format(server_public_ip_address,current_name,cam_no,str(cam_time)),
                            #             "cam_no":cam_no,
                            #             "response2":response2.replace(" ","%20")
                            # }
