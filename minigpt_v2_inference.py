...
from  MiniGPT4.minigpt4.conversation.conversation import Conversation, SeparatorStyle, Chat
...
def parse_args():
    ...
    parser.add_argument("--cfg-path", default='./MiniGPT-4/eval_configs/minigptv2_eval.yaml',
...
while (True):
    try :
        ...
        if method_frame:
            ...
            try:
                infer_a= time.time()
                gr_img ='/root/cctv_ai_and_opensource_vision_llms_for_ai.bits/inputs/{}.png'.format(current_name)
                chat_state = CONV_VISION.copy()
                img_list= []
                user_message= '[detection] fire'
                ...
                llm_message = chat.answer(conv=chat_state,
                                            ...
                                            max_length=2000)[0]
                # print(llm_message)
                print(llm_message,current_name,"first turn <<<<<<<<<<<<<<<,-------------")
                pattern = r"<box>(.*?)</box>"
