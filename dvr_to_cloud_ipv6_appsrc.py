...
class RealPlayDemo:
    def __init__(self):
        ...
        self.pipeline = Gst.parse_launch("""appsrc emit-signals=True is-live=True name=appsrc1 ! h264parse ! rtph264pay ! udpsink host=127.0.0.1 port=12001""")

        self.pipeline._on_pipeline_init = on_pipeline_init.__get__(self.pipeline)

        # self.pipeline.startup()
        self.appsrc = self.pipeline.get_by_name ('appsrc1')
        ...
        main_loop_thread = Thread(target = self.loop.run)
        main_loop_thread.start()

        print(self.channel_1,self.channel_10)
    ...
    def RealDataCallBack(self, lRealHandle, dwDataType, pBuffer, dwBufSize, param, dwUser):
        ...
        if lRealHandle == self.playID:
            if dwDataType == 0:
                ...
                self.appsrc.emit("push-buffer", Gst.Buffer.new_wrapped(data_buffer))
...
if __name__ == '__main__':
    try:
        ...
        print(result)
        if not result:
            my_demo.pipeline.set_state(Gst.State.NULL)
            my_demo.quit_demo()
        else:
