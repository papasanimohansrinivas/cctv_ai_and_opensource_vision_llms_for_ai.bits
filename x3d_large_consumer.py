...
parameters = pika.ConnectionParameters('10.1.96.3',
                                       5672,
                                       '/',
                                       credentials)
...
def create_mp4_file(file_name,np_array):
    ...
    with ImageSequenceClip(list(np_array), fps=2) as clip:
        clip.write_videofile("/root/test_results/"+file_name,fps=2,logger=None)
    clip.close()
...
PATH = "/root/pytorchvideo/x3d_large_model_trial_version_3.pth"
        ...
        try :
            ...
            if method_frame:
                ...
                try:
                    ...
                    if pred.eq(torch.tensor([0]).cuda()) and model_out.cpu().numpy().tolist()[0][0]>0.73:
                        ...
                        dict_["cam_{}".format(str(cam_no))]["timestamps"].append(cam_time)
                        if len(dict_["cam_{}".format(str(cam_no))]["timestamps"])>=1:
                            ...
                            # if (time_2-time_1)<17:
                            dict_["cam_{}".format(str(cam_no))]["to_trigger"]=1
                        #print(" cam no current :- ","cam_{}".format(str(cam_no)),dict_["cam_{}".format(str(cam_no))]," <-----condsider")
                        print(" cam no current :- ","cam_{}".format(str(cam_no))," <-----condsider")
                        output_dict = dict_["cam_{}".format(str(cam_no))]
...
#                            temp_thread.start()
#                            dict_["cam_{}".format(str(cam_no))]["is_alarm_triggered"]=1
