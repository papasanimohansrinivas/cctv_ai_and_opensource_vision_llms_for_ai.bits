        ...
        try :
            ...
            if method_frame:
                ...
                try:
                    ...
                    model_out = model_fire(cv2.cvtColor(input_numpy[0,:,:], cv2.COLOR_BGR2RGB))
                    infer_b = time.time()
                    ...
                    is_ok = False
                    print(cam_no," ",cam_time,"  ",model_out.__str__())
                    for lst_values in model_out.pred[0].cpu().numpy().tolist():
                        print("fire detected investigate0000 <--------",cam_no," ",lst_values[-2]," ",model_out.__str__())
                        if lst_values[-2]> threshold:
                            is_ok = True
                            print("fire detected more than threshold <--------",cam_no," ",lst_values[-2])

                    if "fire" in model_out.__str__() and is_ok:
...
#                           temp_thread.start()
                    ...
                    if  "fire" in model_out.__str__() and is_ok:
                        analysis_array.append([cam_no,cam_time,"fire",model_out.pred[0].cpu().numpy().tolist()[0][-2]])         
                    else:
                        analysis_array.append([cam_no,cam_time,"nofire",0])
#                        with open('./test_results/{}_{}_nofire_pred.npy'.format(cam_no,str(cam_time)), 'wb') as f:
#                            np.save(f,input_numpy)

                    print("inference time !",((infer_b-infer_a)+(log_time_b-log_time_a)),"prediction finally !",model_out.__str__())
